...
var widthdecrconstant = 20;
var heightdecrconstant = 120;
var channel = 0;
...
console.log('videoTilesDevices',videoTilesDevices);
function onReady() {
    initPlayerExample();
}
...
function userLogin(){
    var livesession = '00faf42b8beb47b5a0f75c751eadbcca';
    var minBufferTime = 0;
    var maxBufferTime = 6;
    var height = $(window).height() - heightdecrconstant;
    var width = $(window).width() - widthdecrconstant;
    $('#cmsv6flash').css('height',height);
    $('#cmsv6flash').css('width',width);
    playLiveVideo(0,livesession,devices[0],channel,1,'Vehicle-'+devices[0],minBufferTime,maxBufferTime);
    $('#Cmsv6H5Video-cmsv6flash-1').hide();
    for(var i=1; i <=35; i++){
        $('#Cmsv6H5Video-cmsv6flash-'+i).remove();
    }
    $('#Cmsv6H5Video-cmsv6flash-0').height(height);
    $('#Cmsv6H5Video-cmsv6flash-0').width(width);
}
function userLogin1() {
